...
import { CircleCheck as CheckCircle2, Clock, ArrowLeft, CreditCard, Wallet, Building2, Zap, Calendar, TrendingDown, CircleAlert as AlertCircle } from "lucide-react";
    ...
    <AppShell>
      <div className="space-y-6">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={cn(
                    ...
                    mode === m.key ? "bg-primary text-white" : "bg-muted",
                  )}>
                ...
                <div className={cn(
                  ...
                  mode === m.key ? "border-primary bg-primary" : "border-border",
                )}>
        ...
        {mode === "partial" && (
          <Card className="p-5">
            <Input
              label="Amount to repay (R)"
              type="number"
              placeholder={`Max: ${(totalOwed / 100).toFixed(2)}`}
              value={amount}
            ...
            <div className="mt-3 flex gap-2">
              {[0.25, 0.5, 0.75].map((pct) => (
                <button
                  key={pct}
                  onClick={() => setAmount(((totalOwed * pct) / 100).toFixed(2))}
            ...
            {methods.map((m) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex flex-col items-start gap-2 rounded-2xl border-2 p-4 transition-all",
                ...
                >
                  <div className={cn(
                    "grid size-10 place-items-center rounded-xl",
                    selectedMethod === m.key ? "bg-primary text-white" : "bg-muted",
                  )}>
                    <Icon size={18} />
                  </div>
                  <div>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-bold tabular-nums">
                {formatRand(mode === "full" ? totalOwed : mode === "partial" ? partialAmountCents : scheduleAmountCents)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">Method</span>
              <span className="font-bold capitalize">{selectedMethod.replace("_", " ")}</span>
            </div>
            <div className="flex justify-between border-t pt-2">
              <span className="font-heading font-black">Remaining after</span>
              <span className="font-heading font-black tabular-nums text-primary">
                {formatRand(Math.max(0, totalOwed - (mode === "full" ? totalOwed : mode === "partial" ? partialAmountCents : scheduleAmountCents)))}
              </span>
            </div>
          </div>
          <Button
            ...
            disabled={mode === "partial" && partialAmountCents <= 0}
          >
            Confirm repayment
          </Button>
        </Card>
        ...
        <div>
          ...
          {sortedRepayments.length === 0 ? (
            <Card className="py-8 text-center">
              ...
              <p className="mt-3 text-sm text-muted-foreground">No repayments yet</p>
            </Card>
          ) : (
            <div className="space-y-3">
                ...
                <Card key={r.id} className="p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold">{formatRand(r.amountCents)}</p>
                        <p className="text-xs text-muted-foreground capitalize">{r.type.replace("_", " ")} · {r.paymentMethod.replace("_", " ")}</p>
                      </div>
                    </div>
                    <span className={cn(
      ...
      <Dialog open={showConfirm} onClose={() => setShowConfirm(false)} title="Confirm repayment">
        <div className="space-y-4">
          ...
          {selectedMethod === "wallet" && (
            <div className="flex items-center gap-2 rounded-xl bg-warning/10 p-3 text-xs text-warning">
              <AlertCircle size={14} className="shrink-0" />
              <span>This will deduct from your wallet balance</span>
            </div>
          )}
